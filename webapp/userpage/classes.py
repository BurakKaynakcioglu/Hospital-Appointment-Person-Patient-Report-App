...
class randevuClass():
    def __init__(self, hastaID, doktorID, randevuTarihi, randevuSaati):
    ...
    def save(self):        
        sql_query = "INSERT INTO userpage_randevular (hastaID_id, doktorID_id, randevuTarihi, randevuSaati) VALUES (%s, %s, %s, %s)"
        params = (self.hastaID, self.doktorID, self.randevuTarihi, self.randevuSaati)

        with connection.cursor() as cursor:
            cursor.execute(sql_query, params)

    def delete(self):
        sql_query = "DELETE FROM userpage_randevular WHERE doktorID_id=%s AND hastaID_id=%s AND randevuTarihi=%s AND randevuSaati=%s"
        params = (self.doktorID, self.hastaID, self.randevuTarihi, self.randevuSaati)

        with connection.cursor() as cursor:
            cursor.execute(sql_query, params)
    
    def update(self, randevuID):
        sql_query = "UPDATE userpage_randevular SET randevuTarihi=%s, randevuSaati=%s WHERE id=%s"
        params = (self.randevuTarihi, self.randevuSaati, randevuID)
        with connection.cursor() as cursor:
            cursor.execute(sql_query, params)


def compareDate(tarih1_str, tarih2_str):
